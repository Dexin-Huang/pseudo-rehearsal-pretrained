...
Robins introduced pseudo-rehearsal as a fix for catastrophic forgetting
in small feedforward networks at a time when the modern continual-learning
toolkit did not exist: no GANs, no VAEs, no large pretrained backbones,
no exemplar buffers. The procedure is mechanical
\citep{robins1995pseudorehearsal}:
\begin{enumerate}
\item At the end of task $t$, snapshot the network weights as a frozen
teacher $f_t$.
\item Sample a pool of random input vectors $\{r_i\}$ from a simple
distribution. The original paper used uniform values per unit.
\item Run the teacher on the pool to get pseudo-targets
$q_i = f_t(r_i)$.
\item During task $t{+}1$, mix $(r_i, q_i)$ pairs into each training
batch alongside the new-task data, with a distillation loss on the
pseudo-pairs in addition to the usual loss on the new data.
\end{enumerate}
The student is nudged to keep producing the teacher's responses on
randomly sampled inputs. No data is stored. No generator is trained. The
intuition is that if the random inputs cover enough of the input space,
distilling the teacher's behavior on them transfers the old decision
boundary. The Connection Science paper reported some benefit on toy MLP
sequential tasks. The method was dismissed for deep classifiers because
random inputs to a network trained from scratch are far off the data
manifold and the teacher's outputs on them carry little information
about the old-task decision boundary. Atkinson et al.\
\citep{atkinson2018pseudorehearsal} revived it for deep RL but the line
otherwise saw little follow-up.
...
The first sweep (six methods, three seeds, 18 runs) returns a negative
result for pseudo-rehearsal. Two natural follow-up hypotheses remain.
...
features, (ii) nearest-neighbor cosine similarity from a 5K random sample
to a 5K real sample, (iii) predictive entropy of the joint head on real vs.\
...
Table~\ref{tab:twobytwo}. The running-average accuracy curves are in
Fig.~\ref{fig:curves}.
...
dominates CE and the student converges to the teacher's behavior on the
pseudo-input distribution; whether that recovers joint accuracy on the
real CIFAR test set depends on how well that distribution identifies the
joint head's old-class decision boundary (Section~\ref{sec:discussion}).
In the explored range the curve has not begun to bend toward joint
accuracy. Even at $\alpha = 50$ the gain over the default $\alpha = 1$
is only 0.04 to 0.05.
